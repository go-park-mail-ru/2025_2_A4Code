...
func (repo *ProfileRepository) FindByID(id int64) (*domain.Profile, error) {
	...
	const query = `
		SELECT 
			...
			s.notification_tolerance, s.language, s.theme, COALESCE(s.signature, '')
		FROM 
			base_profile bp
		JOIN 
	...
	err = stmt.QueryRow(id).Scan(
		...
		&profile.Settings.NotificationTolerance, &profile.Settings.Language, &profile.Settings.Theme, &profile.Settings.Signature,
	)
...
func (repo *ProfileRepository) FindInfoByID(profileID int64) (domain.ProfileInfo, error) {
	...
	err = stmt.QueryRow(profileID).Scan(
		...
		&profileInfo.Settings.NotificationTolerance, &profileInfo.Settings.Language, &profileInfo.Settings.Theme, &profileInfo.Settings.Signature,
	)
