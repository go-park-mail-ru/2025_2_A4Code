-- +migrate Up
-- Single-table profiles schema (internal + external)
...
CREATE TYPE profile_kind AS ENUM ('internal','external');
...
CREATE TABLE profile (
    ...
    kind            profile_kind NOT NULL DEFAULT 'internal',
...
CREATE TABLE message (
    ...
    sender_profile_id INTEGER NOT NULL REFERENCES profile(id) ON DELETE NO ACTION,
...
FOR EACH ROW EXECUTE PROCEDURE update_updated_at();
ALTER TABLE thread
    ADD CONSTRAINT fk_thread_root_message FOREIGN KEY (root_message_id) REFERENCES message(id) ON DELETE SET NULL;
...
CREATE TABLE folder_profile_message (
    message_id INTEGER NOT NULL REFERENCES message(id) ON DELETE CASCADE,
